...
"#)
    ...
    temp.child("src/main.rs").write_str("fn main() { println!(\"Hello, world!\"); }").unwrap();
    ...
    temp.child("src/bin/test-project.rs").assert(predicate::path::exists());
    ...
    temp.child("migrations/20231210151519_AddJobsTable.sql").assert(predicate::path::exists());
    temp.child("migrations/20240228040146_AddCrons.up.sql").assert(predicate::path::exists());
    temp.child("migrations/20240228040146_AddCrons.down.sql").assert(predicate::path::exists());
    temp.child("migrations/20250413182934_AddSessions.up.sql").assert(predicate::path::exists());
    temp.child("migrations/20250413182934_AddSessions.down.sql").assert(predicate::path::exists());
...
"#)
    ...
    temp.child("src").create_dir_all().unwrap();
    temp.child("src/lib.rs").write_str("pub fn hello() {}").unwrap();
    ...
    cmd.args(["init", "--bin-name", "web-server"])
        ...
        .assert()
        .success()
        .stdout(predicate::str::contains("Binary created at: src/bin/web-server.rs"))
        .stdout(predicate::str::contains("cargo run --bin web-server"));
    
    // Verify custom binary was created
    temp.child("src/bin/web-server.rs").assert(predicate::path::exists());
...
"#)
    ...
    temp.child("build.rs").assert(predicate::path::exists().not());
...
"#)
    ...
    temp.child("build.rs").write_str(custom_build_content).unwrap();
...
"#)
    ...
    temp.child("my-app/src/main.rs").write_str("fn main() {}").unwrap();
    ...
    temp.child("my-app/src/bin/my-app.rs").assert(predicate::path::exists());
    temp.child("my-app/migrations").assert(predicate::path::is_dir());
}
...
"#)
    ...
    cmd.arg("init")
        ...
        .assert()
        .failure()
        .stderr(predicate::str::contains("Could not find package name in Cargo.toml"));
}
...
"#)
    ...
    temp.child("migrations").assert(predicate::path::exists().not());
...
"#)
    ...
    temp.child("migrations/existing.sql").write_str("-- Existing migration").unwrap();
    ...
    temp.child("migrations/existing.sql").assert(predicate::path::exists());
