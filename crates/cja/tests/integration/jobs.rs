...
async fn test_stale_lock_recovery() {
    ...
    let new_worker_id = "new-worker";
    let lock_timeout_secs: i64 = 60; // 60 seconds for testing

    // Insert a job that was locked more than lock_timeout_secs ago (simulating a crashed worker)
    sqlx::query(
        "INSERT INTO jobs (job_id, name, payload, priority, run_at, created_at, context, error_count, locked_by, locked_at)
    ...
    .bind(job_id)
    .bind("TestJob")
    .bind(serde_json::json!({"id": "stale-lock-test", "value": 1}))
    .bind(0)
    ...
    // Verify job is locked by the stale worker
    let row = sqlx::query("SELECT locked_by FROM jobs WHERE job_id = $1")
        .bind(job_id)
        .fetch_one(&pool)
        .await
        .unwrap();
    assert_eq!(
        row.get::<Option<String>, _>("locked_by"),
        Some(stale_worker_id.to_string())
    );

    // Simulate the fetch_next_job logic that should pick up stale locked jobs
    // This is the same query used in the worker's fetch_next_job method
    ...
    // Verify the job was picked up by the new worker
    assert!(result.is_some());
    let picked_job_id = result.unwrap().get::<Uuid, _>("job_id");
    assert_eq!(picked_job_id, job_id);

    // Verify the lock was transferred to the new worker
    let row = sqlx::query("SELECT locked_by FROM jobs WHERE job_id = $1")
        .bind(job_id)
        .fetch_one(&pool)
        .await
    ...
    assert_eq!(
        row.get::<Option<String>, _>("locked_by"),
        Some(new_worker_id.to_string())
    );
}

#[tokio::test]
async fn test_recently_locked_job_not_picked_up() {
    let (pool, _guard) = crate::common::db::setup_test_db().await.unwrap();

    let job_id = uuid::Uuid::new_v4();
    let active_worker_id = "active-worker";
    let new_worker_id = "new-worker";
    let lock_timeout_secs: i64 = 3600; // 1 hour timeout

    // Insert a job that was locked recently (10 seconds ago, well within the timeout)
    sqlx::query(
        "INSERT INTO jobs (job_id, name, payload, priority, run_at, created_at, context, error_count, locked_by, locked_at)
    ...
    .bind(job_id)
    .bind("TestJob")
    .bind(serde_json::json!({"id": "recent-lock-test", "value": 1}))
    .bind(0)
    ...
    // Try to pick up the job - should NOT be picked up since lock is still fresh
    let result = sqlx::query(
        "UPDATE jobs
         SET locked_by = $1, locked_at = NOW()
         WHERE job_id = (
    ...
    // Verify no job was picked up
    assert!(result.is_none());

    // Verify the original worker still has the lock
    let row = sqlx::query("SELECT locked_by FROM jobs WHERE job_id = $1")
        .bind(job_id)
        .fetch_one(&pool)
        .await
    ...
    assert_eq!(
        row.get::<Option<String>, _>("locked_by"),
        Some(active_worker_id.to_string())
    );
}

#[tokio::test]
async fn test_unlocked_job_preferred_over_stale_locked() {
    let (pool, _guard) = crate::common::db::setup_test_db().await.unwrap();

    let unlocked_job_id = uuid::Uuid::new_v4();
    let stale_locked_job_id = uuid::Uuid::new_v4();
    let new_worker_id = "new-worker";
    let lock_timeout_secs: i64 = 60;

    // Insert an unlocked job (higher priority)
    sqlx::query(
    ...
    .bind("TestJob")
    .bind(serde_json::json!({"id": "unlocked", "value": 1}))
    ...
    // Insert a stale locked job (lower priority)
    sqlx::query(
        "INSERT INTO jobs (job_id, name, payload, priority, run_at, created_at, context, error_count, locked_by, locked_at)
         VALUES ($1, $2, $3, $4, NOW(), NOW(), $5, $6, $7, NOW() - interval '120 seconds')",
    )
    ...
    .bind("TestJob")
    .bind(serde_json::json!({"id": "stale-locked", "value": 2}))
    ...
    // Pick up a job - should get the higher priority unlocked one
    let result = sqlx::query(
        "UPDATE jobs
         SET locked_by = $1, locked_at = NOW()
         WHERE job_id = (
    ...
    // Should pick the unlocked job (higher priority)
    assert!(result.is_some());
    let picked_job_id = result.unwrap().get::<Uuid, _>("job_id");
    assert_eq!(picked_job_id, unlocked_job_id);
}
