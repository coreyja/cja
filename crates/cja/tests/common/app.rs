...
async fn session_handler<AS: AppState>(
    State(_state): State<AS>,
) -> &'static str {
    "Session OK"
}
...
impl TestClient {
    ...
    pub fn cookie_value(&self, name: &str) -> Option<String> {
        ...
        let header_str = self.cookies.cookies(&url)?
            .to_str()
            .ok()?
            .to_string();
...
pub async fn spawn_test_server<AS: AppState + 'static>(router: Router) -> (TestClient, tokio::task::JoinHandle<()>) {
